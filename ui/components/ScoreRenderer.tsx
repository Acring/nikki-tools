...
const ScoreRenderer: React.FC<ScoreRendererProps> = ({ score }) => {
        ...
        <span className="inline-block mx-1 relative">
          <span className="relative">
            ...
            {(note.pitch?.octaveShift ?? 0) > 0 &&
              [...Array(note.pitch?.octaveShift)].map((_, index) => (
                <span
                  key={`octave-up-${index}`}
            ...
            {/* 添加低八度圆点标记 */}
            {(note.pitch?.octaveShift ?? 0) < 0 &&
              [...Array(Math.abs(note.pitch?.octaveShift ?? 0))].map((_, index) => (
                <span
          ...
          <span className="relative">{getDurationDisplay().text}</span>
        </span>
      );
    }
  ...
  return (
    <div className="score-renderer">
      ...
      <div className="mb-4">
        {score.header.titles &&
          score.header.titles.map((title, index) => (
      ...
      <div className="p-4 border rounded bg-white font-mono text-lg">
        {score.notes.map((note, index) => (
          <React.Fragment key={index}>{renderNote(note)}</React.Fragment>
        ))}
      </div>
    </div>
  );
